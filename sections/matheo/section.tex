...
Der Advanced Encryption Standard (AES) ist ein symmetrischer Blockchiffre-Algorithmus zur Verschlüsselung von Daten wie VPN, HTTPS und WLAN (WPA2). Er wurde 2001 vom NIST entwickelt und nutzt Schlüssellängen von 128, 192 oder 256 Bit, wobei längere Schlüssel sicherer sind. Die Blockgröße beträgt stets 128 Bit. Die Anzahl der Runden hängt von der Schlüssellänge ab:  
\begin{itemize}  
...
Der Schlüssel wird durch Verfahren wie Password-Based Encryption (PBE) oder Diffie-Hellman bestimmt, da AES keine eigene Schlüsselverteilung unterstützt.  
...
Jede Runde umfasst \textit{SubBytes}, \textit{ShiftRows}, \textit{MixColumns} und \textit{Add Round Key}. In der letzten Runde entfällt \textit{MixColumns}.\cite{AES_Algorithmus_2}\cite{AES_Algorithmus_3}\cite{AES_Algorithmus}\cite{Blockchiffre}  


\subsection{AES Ablauf Beispiel}

 % Adds space before the PDF
\begin{figure}
	\centering
	\includegraphics[width=0.40\textwidth]{sections/matheo/final.jpg}
	\caption{test wird morgen fertig gemacht}
...
\subsection{Sicherheit gegenüber Quantenangriffen}
